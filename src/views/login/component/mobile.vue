<template>
	<el-form size="large" class="login-content-form" ref="loginFormRef" :rules="loginRules" :model="loginForm" @keyup.enter="handleLogin">
		...
		<el-form-item class="mt-4 login-animation3">
			<el-button type="primary" class="rounded-lg login-content-submit" v-waves @click="handleLogin" :loading="loading">
				<span class="font-semibold tracking-wide">{{ $t('mobile.btnText') }}</span>
			</el-button>
		</el-form-item>
...
import { LoginTypeEnum, sendMobileCode } from '@cqdcg/admin/api/login/index';
import { useMessage } from '@cqdcg/admin/hooks/message';
import { useUserInfo } from '@cqdcg/admin/stores/userInfo';
import { rule } from '@cqdcg/admin/utils/validate';
