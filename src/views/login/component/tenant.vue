...
import { fetchList } from '@cqdcg/admin/api/admin/tenant';
...
// 是否开启注册
const autoTenantEnable = ref(ViteEnv.env.VITE_AUTO_TENANT === 'true');
...
const initTenantConfig = () => {
	...
	tenantList.value
		...
		.forEach((tenant: any) => {
			themeConfig.value.globalTitle = tenant?.websiteName || ViteEnv.env.VITE_GLOBAL_TITLE;

			themeConfig.value.footerAuthor = tenant.footer || ViteEnv.env.VITE_FOOTER_TITLE;
